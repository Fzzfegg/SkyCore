package org.mybad.core.expression.molang.reference;

import org.mybad.core.expression.molang.ast.MolangExpression;
import org.mybad.core.expression.molang.ast.ObjectAwareExpression;
...
public class ExpressionBindingContext {
    ...
    private static class FunctionBinder<T> implements Binder<T> {
        private final Class<T> value;
        private final String expression;
        private final Function<T, ObjectAwareExpression<T>> mapper;

        public FunctionBinder(Class<T> value, String expression, Function<T, ObjectAwareExpression<T>> mapper) {
            this.value = value;
    ...
    private static class SupplierBinder<T> implements Binder<T> {
        private final String expression;
        private final Supplier<ObjectAwareExpression<Void>> mapper;

        public SupplierBinder(String expression, Supplier<ObjectAwareExpression<Void>> mapper) {
            this.expression = expression;
            this.mapper = mapper;
        }
