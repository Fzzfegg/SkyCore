...
import org.mybad.minecraft.gltf.jgltf.model.GltfModel;
import org.mybad.minecraft.gltf.jgltf.model.io.GltfModelReader;
...
public class GltfDataModel {
    ...
    private static final Comparator<DataAnimation.DataKeyframe> COMPARATOR_ANI = new Comparator() {

        @Override
        public int compare(Object o1, Object o2) {
            // TODO Auto-generated method stub
            return ((DataAnimation.DataKeyframe)o1).time > ((DataAnimation.DataKeyframe)o2).time ? 1 : -1;
        }
    ...
    public static GltfDataModel load(ResourceLocation loc) {
        ...
        try (InputStream inputStream = GltfResourceAccess.open(loc)) {
            if (inputStream == null) {
            ...
            model.getMaterialModels().forEach((materialModel) -> {
                ...
                if (map != null && map.containsKey("isTranslucent")) {
            ...
            /**
             * Nodes begining - First pass: collect all node names and find duplicates
             */
            Map<String, Integer> nodeNameCounts = new HashMap<>();
            Set<String> duplicateNames = new HashSet<>();

            // 第一遍扫描：统计所有节点名称出现次数
            model.getNodeModels().forEach((nodeModel) -> {
                String name = nodeModel.getName();
                nodeNameCounts.put(name, nodeNameCounts.getOrDefault(name, 0) + 1);
            });

            // 找出所有重复的节点名称
            nodeNameCounts.entrySet().forEach(entry -> {
                if (entry.getValue() > 1) {
                    duplicateNames.add(entry.getKey());
                    System.err.println("[GltfDataModel] Found duplicate node name: '" + entry.getKey() + "' (appears " + entry.getValue() + " times)");
                }
            });

            if (!duplicateNames.isEmpty()) {
                System.err.println("[GltfDataModel] ========== ALL DUPLICATE NODES ==========");
                duplicateNames.forEach(name -> {
                    System.err.println("[GltfDataModel] - '" + name + "' appears " + nodeNameCounts.get(name) + " times");
                });
                System.err.println("[GltfDataModel] =======================================");
                System.err.println("[GltfDataModel] Auto-renaming duplicates to continue loading...");
            }
            ...
            model.getNodeModels().forEach((nodeModel) -> {
                ...
                String originalName = nodeModel.getName();
                String nodeName = originalName;

                if (gltfDataModel.nodes.containsKey(nodeName)) {
                    // 这是重复节点，需要重命名
                    int counter = duplicateCounters.getOrDefault(originalName, 1);
                    duplicateCounters.put(originalName, counter + 1);

                    nodeName = originalName + "_duplicate_" + counter;
                    while (gltfDataModel.nodes.containsKey(nodeName)) {
                        counter++;
                        nodeName = originalName + "_duplicate_" + counter;
                    }

                    System.err.println("[GltfDataModel] Renamed '" + originalName + "' to '" + nodeName + "'");
                }

                gltfDataModel.nodes.put(nodeName, node);
                if (nodeModel.getParent() == null) {
                    gltfDataModel.rootNodes.put(nodeName, node);
                }
                node.name = nodeName;
                if (nodeModel.getParent() != null) {
                ...
                nodeModel.getMeshModels().forEach((meshGModel) -> {
                    meshGModel.getMeshPrimitiveModels().forEach((meshModel) -> {
                        ...
                        String materialName = meshModel.getMaterialModel() == null
                            ? "###DEFAULT###"
                            : meshModel.getMaterialModel().getName();
                        dataMesh.material = materialName;
                        node.meshes.put(materialName, dataMesh);
                        ...
                        if (isSkining) {
                            ...
                            while (buffer.hasRemaining()) {
                                ...
                                // 边界保护检查
                                if (point < 0 || point >= posList.size()) {
                                    System.out.println("[GltfDataModel] WARNING: Invalid index in skinned mesh! Index: " + point + ", valid range: 0-" + (posList.size()-1));
                                    continue; // 跳过无效索引
                                }
                            ...
                            while (buffer.hasRemaining()) {
                                ...
                                // 边界保护检查
                                if (point < 0 || point >= posList.size()) {
                                    System.out.println("[GltfDataModel] WARNING: Invalid index in static mesh! Index: " + point + ", valid range: 0-" + (posList.size()-1));
                                    continue; // 跳过无效索引
                                }
            ...
            if (shouldLogLoadError(loc.toString())) {
                System.out.println("[GltfDataModel] Exception during loading: " + loc + " " + gltfDataModel.lastPos);
                System.out.println("[GltfDataModel] Error message: " + e.getMessage());
                System.out.println("Something is wrong when loading:" + loc +" "+gltfDataModel.lastPos);
                e.printStackTrace();
            }
        }
    ...
    private static final Map<String, LogLimiter> LOAD_ERROR_LIMITER = new HashMap<>();

    private static boolean shouldLogLoadError(String loc) {
        long now = System.currentTimeMillis();
        LogLimiter limiter = LOAD_ERROR_LIMITER.computeIfAbsent(loc, k -> new LogLimiter());
        cleanupLimiter(now);
        if (limiter.count >= 10) {
            return false;
        }
    ...
    private static void cleanupLimiter(long now) {
        if (LOAD_ERROR_LIMITER.size() <= 2048) {
            return;
        }
        LOAD_ERROR_LIMITER.entrySet().removeIf(e -> (e.getValue().count >= 10 && now - e.getValue().lastLogMs > 60_000L)
            || now - e.getValue().lastLogMs > 10 * 60_000L
            || LOAD_ERROR_LIMITER.size() > 4096);
    }
