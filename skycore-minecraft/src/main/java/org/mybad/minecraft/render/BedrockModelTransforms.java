 ...
 */
final class BedrockModelTransforms {
    private static final float PIXEL_SCALE = 1.0f / 16.0f;

    private BedrockModelTransforms() {
    }

    static float convertX(float raw) {
        return -raw * PIXEL_SCALE;
    }

    static float convertY(float raw) {
        return raw * PIXEL_SCALE;
    }

    static float convertZ(float raw) {
        return raw * PIXEL_SCALE;
    }

    static float convertLocatorX(float raw) {
        return -raw * PIXEL_SCALE;
    }

    static float convertLocatorY(float raw) {
        return raw * PIXEL_SCALE;
    }

    static float convertLocatorZ(float raw) {
        return raw * PIXEL_SCALE;
    }

    static float convertRotation(float raw, boolean invert) {
        return invert ? -raw : raw;
    }

    static void applyBoneTransform(ModelBone bone, MatrixStack stack) {
    ...
    static void applyBoneTransformRecursive(ModelBone bone, MatrixStack stack) {
    ...
    static MatrixStack buildCubeRotationStack(ModelCube cube) {
        if (!cube.hasRotation()) {
            return null;
        }
