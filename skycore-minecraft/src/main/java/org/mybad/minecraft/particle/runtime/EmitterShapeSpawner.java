...
final class EmitterShapeSpawner implements ParticleEmitterShape.Spawner {
    ...
    public void setPosition(ParticleInstance instance, double x, double y, double z) {
        ...
        boolean rotatePosition = localPos || emitter.isLocatorBound() || emitter.isLocalRotation();
        float scale = emitter.getScale();
        x *= scale;
        y *= scale;
        z *= scale;
        double rx = rotatePosition ? emitter.rotateLocalX(x, y, z) : x;
        double ry = rotatePosition ? emitter.rotateLocalY(x, y, z) : y;
        double rz = rotatePosition ? emitter.rotateLocalZ(x, y, z) : z;
    ...
    public void setVelocity(ParticleInstance instance, double dx, double dy, double dz) {
        ...
        boolean localVelocity = emitter.isLocalVelocity();
        boolean rotateVelocity = localVelocity || emitter.isLocalRotation() || emitter.isLocatorBound();
        float scale = emitter.getScale();
        dx *= scale;
        dy *= scale;
        dz *= scale;
        double rx = rotateVelocity ? emitter.rotateLocalX(dx, dy, dz) : dx;
        double ry = rotateVelocity ? emitter.rotateLocalY(dx, dy, dz) : dy;
        double rz = rotateVelocity ? emitter.rotateLocalZ(dx, dy, dz) : dz;
        double vx = rx;
        double vy = ry;
        double vz = rz;
        if (!localVelocity) {
            vx += emitter.getDeltaX();
            vy += emitter.getDeltaY();
            vz += emitter.getDeltaZ();
        }
        ActiveParticle particle = (ActiveParticle) instance;
