...
public interface ParticleEvent {
    ...
    class Deserializer implements JsonDeserializer<ParticleEvent> {
        ...
        private static ParticleEvent[] parseSequence(@Nullable JsonElement element, JsonDeserializationContext context) {
            ...
            JsonArray array = ParticleGsonHelper.convertToJsonArray(element, "sequence");
            if (array.size() == 0) {
        ...
        private static @Nullable ParticleEvent parseRandom(@Nullable JsonElement element, JsonDeserializationContext context) {
            ...
            JsonArray array = ParticleGsonHelper.convertToJsonArray(element, "randomize");
            if (array.size() == 0) {
            ...
            for (int i = 0; i < array.size(); i++) {
                JsonObject arrayElement = ParticleGsonHelper.convertToJsonObject(array.get(i), "randomize[" + i + "]");
                events[i] = context.deserialize(arrayElement, ParticleEvent.class);
                weights[i] = ParticleGsonHelper.getAsInt(arrayElement, "weight", 0);
            }
        ...
        public ParticleEvent deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
            JsonObject jsonObject = ParticleGsonHelper.convertToJsonObject(json, "event");
            ...
            if (jsonObject.has("expression")) {
                events.add(expression(ParticleJsonTupleParser.getExpression(jsonObject, "expression", null)));
            }
            if (jsonObject.has("log")) {
                events.add(log(ParticleGsonHelper.getAsString(jsonObject, "log")));
            }
            if (events.isEmpty()) {
