...
public final class ParticleKillPlaneComponent implements ParticleComponent {
    ...
    public static ParticleKillPlaneComponent deserialize(JsonElement json) throws JsonParseException {
        ...
        if (!json.isJsonArray()) {
            throw new JsonSyntaxException("Expected minecraft:particle_kill_plane to be a JsonArray, was " + ParticleGsonHelper.getType(json));
        }
        ...
        for (int i = 0; i < coefficients.length; i++) {
            coefficients[i] = ParticleGsonHelper.convertToFloat(vectorJson.get(i), "minecraft:particle_kill_plane[" + i + "]");
        }
