...
public final class Flipbook {
    ...
    public static class Deserializer implements JsonDeserializer<Flipbook> {
        ...
        public Flipbook deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
            ...
            MolangExpression[] uv = ParticleJsonTupleParser.getExpression(jsonObject, "base_UV", 2, null);
            float[] sizeUV = ParticleJsonTupleParser.getFloat(jsonObject, "size_UV", 2, null);
            float[] stepUV = ParticleJsonTupleParser.getFloat(jsonObject, "step_UV", 2, null);
            float fps = ParticleGsonHelper.getAsFloat(jsonObject, "frames_per_second", 1);
            MolangExpression maxFrame = ParticleJsonTupleParser.getExpression(jsonObject, "max_frame", null);
            boolean stretchToLifetime = ParticleGsonHelper.getAsBoolean(jsonObject, "stretch_to_lifetime", false);
            boolean loop = ParticleGsonHelper.getAsBoolean(jsonObject, "loop", false);
