...
public final class ParticleInitialSpeedComponent implements ParticleComponent {
    ...
    public static ParticleInitialSpeedComponent deserialize(JsonElement json) throws JsonParseException {
        if (json.isJsonPrimitive()) {
            MolangExpression expression = ParticleJsonTupleParser.parseExpression(json, "speed");
        ...
        if (json.isJsonArray()) {
            ...
            MolangExpression dx = ParticleJsonTupleParser.parseExpression(jsonArray.get(0), "speed[0]");
            MolangExpression dy = ParticleJsonTupleParser.parseExpression(jsonArray.get(1), "speed[1]");
            MolangExpression dz = ParticleJsonTupleParser.parseExpression(jsonArray.get(2), "speed[2]");
            MolangExpression[] speed = new MolangExpression[]{dx, dy, dz};
            return new ParticleInitialSpeedComponent(speed);
        }

        throw new JsonSyntaxException("Expected speed to be a JsonArray or float, was " + ParticleGsonHelper.getType(json));
    }
}
