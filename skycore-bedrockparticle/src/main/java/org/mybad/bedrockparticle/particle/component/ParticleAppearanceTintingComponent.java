...
public final class ParticleAppearanceTintingComponent implements ParticleComponent {
    ...
    public static ParticleAppearanceTintingComponent deserialize(JsonElement json) throws JsonParseException {
        ...
        if (colorElement.isJsonObject()) {
            ...
            MolangExpression interpolant = ParticleJsonTupleParser.getExpression(colorObject, "interpolant", null);
            ...
            if (gradientElement.isJsonObject()) {
                ...
                throw new JsonSyntaxException("Expected gradient to be a JsonObject or JsonArray, was " + ParticleGsonHelper.getType(gradientElement));
            }
            ...
            throw new JsonSyntaxException("Expected color to be a JsonObject, JsonArray, string, or float, was " + ParticleGsonHelper.getType(colorElement));
        }
    ...
    private static ColorSupplier[] parseColor(@Nullable JsonElement json, String name) throws JsonParseException {
        ...
        if (json.isJsonArray()) {
            ...
            for (int i = 0; i < colorJson.size(); i++) {
                ...
                if (!colorElement.isJsonPrimitive()) {
                    throw new JsonSyntaxException("Expected " + name + "[" + i + "] to be a string or float, was " + ParticleGsonHelper.getType(colorElement));
                }
                if (colorElement.getAsJsonPrimitive().isNumber()) {
                    colors[i] = ColorSupplier.constant(ParticleGsonHelper.convertToFloat(colorElement, name + "[" + i + "]"));
                } else {
                    try {
                        MolangExpression expression = ParticleMolangCompiler.get().compile(ParticleGsonHelper.convertToString(colorElement, name + "[" + i + "]"));
            ...
            String value = ParticleGsonHelper.convertToString(json, name);
        ...
        throw new JsonSyntaxException("Expected " + name + " to be a JsonArray or string, was " + ParticleGsonHelper.getType(json));
    }
