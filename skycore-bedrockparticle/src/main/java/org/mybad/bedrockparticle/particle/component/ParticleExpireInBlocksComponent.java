 ...
 * @since 1.0.0
 */
public final class ParticleExpireInBlocksComponent implements ParticleComponent {

    private final String[] blocks;

    public ParticleExpireInBlocksComponent(String[] blocks) {
        this.blocks = blocks;
    }
    ...
    public static ParticleExpireInBlocksComponent deserialize(JsonElement json) throws JsonParseException {
        ...
        try {
            for (int i = 0; i < jsonObject.size(); i++) {
                blocks[i] = ParticleGsonHelper.convertToString(jsonObject.get(i), "minecraft:particle_expire_if_in_blocks[" + i + "]");
            }
        } catch (Exception e) {
