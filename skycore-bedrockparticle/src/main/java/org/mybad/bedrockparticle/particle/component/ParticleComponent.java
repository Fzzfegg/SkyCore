...
public interface ParticleComponent {
    ...
    static String[] parseEvents(@Nullable JsonElement element, String name) throws JsonSyntaxException {
        ...
        if (element.isJsonArray()) {
            ...
            for (int i = 0; i < array.size(); i++) {
                events[i] = ParticleGsonHelper.convertToString(array.get(i), name + "[" + i + "]");
            }
            return events;
        } else if (element.isJsonPrimitive()) {
            return new String[]{ParticleGsonHelper.convertToString(element, name)};
        }
        throw new JsonSyntaxException("Expected " + name + " to be a JsonArray or string, was " + ParticleGsonHelper.getType(element));
    }
    ...
    static Either<Boolean, MolangExpression[]> parseDirection(JsonObject json, String name) {
        ...
        if (json.get(name).isJsonPrimitive()) {
            String directionString = ParticleGsonHelper.getAsString(json, name);
        ...
        return Either.right(ParticleJsonTupleParser.getExpression(json, name, 3, null));
    }
