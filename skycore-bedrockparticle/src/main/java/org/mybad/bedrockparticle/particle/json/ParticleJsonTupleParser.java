 ...
 * @since 1.0.0
 */
public interface ParticleJsonTupleParser {
    ...
    static float[] getFloat(JsonObject json, String name, int length, @Nullable Supplier<float[]> defaultValue) throws JsonSyntaxException {
        ...
        if (!json.has(name)) {
            throw new JsonSyntaxException("Expected " + name + " to be a JsonArray or JsonPrimitive, was " + ParticleGsonHelper.getType(json));
        }
        ...
        if (json.get(name).isJsonArray()) {
            ...
            if (vectorJson.size() == 1) {
                Arrays.fill(values, ParticleGsonHelper.convertToFloat(vectorJson.get(0), name));
            } else {
                for (int i = 0; i < values.length; i++) {
                    values[i] = ParticleGsonHelper.convertToFloat(vectorJson.get(i), name + "[" + i + "]");
                }
            }
    ...
    static MolangExpression[] getExpression(JsonObject json, String name, int length, @Nullable Supplier<MolangExpression[]> defaultValue) throws JsonSyntaxException {
        ...
        if (json.get(name).isJsonArray()) {
            ...
            if (vectorJson.size() == 1) {
                Arrays.fill(values, ParticleJsonTupleParser.parseExpression(vectorJson.get(0), name));
            } else {
                for (int i = 0; i < values.length; i++) {
                    values[i] = ParticleJsonTupleParser.parseExpression(vectorJson.get(i), name + "[" + i + "]");
                }
            }
        ...
        if (json.get(name).isJsonPrimitive()) {
            MolangExpression expression = ParticleJsonTupleParser.getExpression(json, name, null);
            MolangExpression[] values = new MolangExpression[length];
    ...
    static MolangExpression getExpression(JsonObject json, String name, @Nullable Supplier<MolangExpression> defaultValue) throws JsonSyntaxException {
        return !json.has(name) && defaultValue != null ? defaultValue.get() : ParticleJsonTupleParser.parseExpression(json.get(name), name);
    }
    ...
    static MolangExpression parseExpression(@Nullable JsonElement json, String name) {
        if (json != null && json.isJsonPrimitive()) {
            ...
            if (valuePrimitive.isNumber()) {
                ...
                try {
                    ...
                    return ParticleMolangCompiler.get().compile(expr);
        ...
        throw new JsonSyntaxException("Expected " + name + " to be a Float or JsonPrimitive, was " + ParticleGsonHelper.getType(json));
    }
